...
- **Custom Vector SVG Icons**: Includes workbench icons for workbench selection (``NodeGraph_Workbench.svg``), object creation (``NodeGraph_Create.svg``), view editor (``NodeGraph_Editor.svg``), graph execution (``NodeGraph_Run.svg``), nodes (``NodeGraph_Node.svg``), and links (``NodeGraph_Link.svg``).
